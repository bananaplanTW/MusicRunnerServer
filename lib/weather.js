...
exports.parseNext24HoursWeather = function (currentHour, data) {
    ...
    for (i = 0; i < 3; i ++) {
        startTime = new Date(weatherElements.MaxT[0].time[i]['$'].start);
        endTime   = new Date(weatherElements.MaxT[0].time[i]['$'].end);
    ...
    for (i = 0; i < 24 && currentIndex < 3; i+=1 ) {
        if (previousIndex !== currentIndex) {
            ...
            console.log(weatherElements.MaxT[0].time);
...
exports.parseWeekWeather = function (data) {
    var weekWeather = [];
    var dateMapping = [];
    var today = new Date(data.Wx[0].time[0]['$'].start);
